...
// ---------------------------------------------------------------- the turn ring's colour
// render.js is drawing code and expects a browser, so it needs a canvas-shaped hole to
// load into. Only the colour maths is under test here; nothing below touches the canvas.
globalThis.Path2D ??= class {};
globalThis.Image ??= class {};
globalThis.document ??= { createElement: () => ({ getContext: () => ({}) }) };
const { readableOnSea, seaAt, SEA_COLORS } = await import('../public/render.js');

check('the turn ring separates from whatever the sea was set to', () => {
  // The sea is a colour wheel now, so it can land on any player's colour. A ring that
  // reads as part of the background is a ring nobody sees, which defeats the whole point
  // of it. The sea is a gradient, so the bar is 3:1 against its middle with a floor of
  // 2:1 at each end — the ring runs around the window and must not fade out along one
  // edge. See readableOnSea for why both ends at 3:1 was too strict to keep.
  const lum = (hex) => {
  ...
  const ratio = (x, y) => {
    const [p, q] = [lum(x) + 0.05, lum(y) + 0.05];
    return p > q ? p / q : q / p;
  };
  const mix = (x, y) => '#' + [1, 3, 5].map((i) => Math.round(
    (parseInt(x.slice(i, i + 2), 16) + parseInt(y.slice(i, i + 2), 16)) / 2,
  ).toString(16).padStart(2, '0')).join('');

  // Every preset, plus a custom sea in each player's own colour — the worst case, and the
  // one a colour wheel makes easy to hit by accident — plus a sweep of greys, which is
  // where black and white are both about as far away as each other.
  const seas = [
    ...SEA_COLORS.map((c) => c.key),
    ...R.PLAYER_COLORS.map((c) => `custom:${c.hex}`),
    ...Array.from({ length: 26 }, (_, i) => {
      const v = (i * 10).toString(16).padStart(2, '0');
      return `custom:#${v}${v}${v}`;
    }),
    'custom:#ff5cb8', 'custom:#bfe6ff', 'custom:#0a1a2b', 'custom:#7fbf7f', 'custom:#2f6ea8',
  ];

  for (const { hex } of R.PLAYER_COLORS) {
    for (const key of seas) {
      const { a: top, b: bot } = seaAt(key);
      const mid = mix(top, bot);
      const got = readableOnSea(hex, key);
      const worst = Math.min(ratio(got, top), ratio(got, bot));
      assert(/^#[0-9a-f]{6}$/i.test(got), `${hex} on ${key} gave ${got}`);
      assert(ratio(got, mid) >= 3 && worst >= 2,
        `${hex} on ${key} came back ${got}, ${ratio(got, mid).toFixed(2)}:1 on the middle `
        + `and ${worst.toFixed(2)}:1 at the worse end`);
    }
  }
});

check('the ring keeps the hue it can afford to keep', () => {
  // Pushed to pure white on every awkward sea, four players get four identical rings and
  // the colour stops saying whose turn it is. Some hue has to survive the shift.
  const hueOf = (h) => {
    const [r, g, b] = [1, 3, 5].map((i) => parseInt(h.slice(i, i + 2), 16) / 255);
    const max = Math.max(r, g, b), min = Math.min(r, g, b), d = max - min;
    if (d < 0.02) return null;                       // grey has no hue to compare
    const h6 = max === r ? ((g - b) / d + 6) % 6 : max === g ? (b - r) / d + 2 : (r - g) / d + 4;
    return h6 * 60;
  };
  for (const key of ['custom:#2f6ea8', 'custom:#6e6e6e', 'lagoon', 'harbour']) {
    const rings = R.PLAYER_COLORS.map((c) => readableOnSea(c.hex, key));
    assert(new Set(rings).size >= R.PLAYER_COLORS.length - 1,
      `${key} collapsed the rings to ${new Set(rings).size} colours`);
    for (const [i, { hex }] of R.PLAYER_COLORS.entries()) {
      const [was, now] = [hueOf(hex), hueOf(rings[i])];
      if (was === null || now === null) continue;
      const off = Math.min(Math.abs(was - now), 360 - Math.abs(was - now));
      assert(off < 12, `${hex} on ${key} came back ${rings[i]}, ${off.toFixed(0)}° off`);
    }
  }
});

check('a sea that already contrasts leaves the player colour alone', () => {
  // Shifting a colour that did not need shifting would hand people a ring that is not
  // quite theirs, which matters when four rings are being told apart by colour.
  eq(readableOnSea('#ffffff', 'custom:#0a1a2b'), '#ffffff', 'white on a night sea');
  eq(readableOnSea('#111111', 'custom:#eef8ff'), '#111111', 'near-black on an ice sea');
});
